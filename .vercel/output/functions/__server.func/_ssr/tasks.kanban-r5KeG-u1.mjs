...
import { u as useAuth, f as useTasks, h as useColumns, H as useUserColumnOrder, J as useUserTaskOrder, e as useClients, j as useProfiles, k as useTaskTags, l as useTaskStatuses, G as useTaskCollaborators, E as useSubtasks, K as useTaskTagLinks, B as Button, I as Input, S as Select, n as SelectTrigger, o as SelectValue, p as SelectContent, q as SelectItem, D as Dialog, a as DialogContent, b as DialogHeader, c as DialogTitle, d as DialogFooter, g as useAssignableProfiles, x as cn, F as FileDropZone, A as AttachmentPreviewDialog, L as Label } from "./router-ZM7179_C.mjs";
...
import { P as Popover, a as PopoverTrigger, b as PopoverContent, c as RichTextEditor, R as RichTextView } from "./RichTextEditor-P9Iw8467.mjs";
...
import { aw as FolderOpen, P as Plus, ax as Rows2, ay as Columns2, q as PanelsTopLeft, az as PanelTop, A as ArrowUp, d as ArrowDown, aA as Settings2, ad as GripVertical, aB as EllipsisVertical, b as Pencil, T as Trash2, C as ChevronDown, r as Users, a1 as CircleCheck, aq as Copy, aC as Ellipsis, at as Clock3, J as Calendar, c as Check, aD as Tag, aE as TextAlignStart, ap as ChevronRight, l as ListChecks, av as Link2, F as FileText, X, a9 as Upload, aF as Flag, aG as History, ag as User, f as Paperclip, E as ExternalLink, aH as EyeOff, aI as Eye, j as Save, aJ as Circle } from "../_libs/lucide-react.mjs";
...
const ALL_FIELDS = ["tags", "description", "subtasks", "attachments", "priority", "due", "createdAt", "meta"];
const FIELD_LABELS = {
  tags: "Etiquetas",
...
const DEFAULT_ORDER = ["tags", "description", "subtasks", "attachments", "priority", "due", "createdAt", "meta"];
const DEFAULT_PREFS = { field_order: DEFAULT_ORDER, hidden_fields: [], kanban_orientation: "vertical" };
function migrateFields(fields) {
  const out = [];
  for (const field of fields) {
    const next = field === "chips" ? ["priority", "due", "createdAt"] : field === "due" ? ["due", "createdAt"] : [field];
    for (const value of next) if (ALL_FIELDS.includes(value) && !out.includes(value)) out.push(value);
  }
  return out;
}
function normalize(prefs) {
  const order = migrateFields(Array.isArray(prefs?.field_order) ? prefs.field_order : []);
  return {
    field_order: [...order, ...DEFAULT_ORDER.filter((field) => !order.includes(field))],
    hidden_fields: migrateFields(Array.isArray(prefs?.hidden_fields) ? prefs.hidden_fields : []),
...
function useUpdateBoardPreferences() {
  ...
  return useMutation({
    mutationFn: async (patch) => {
      ...
      const next = { ...qc.getQueryData(["board_preferences", user.id]) ?? DEFAULT_PREFS, ...patch };
      const { error } = await supabase.from("board_preferences").upsert({ user_id: user.id, ...next }, { onConflict: "user_id" });
      if (error) throw error;
      return next;
    },
    onMutate: async (patch) => {
      if (user) qc.setQueryData(["board_preferences", user.id], (current) => ({ ...current ?? DEFAULT_PREFS, ...patch }));
    },
    onSettled: () => {
...
function TaskCard({
  ...
  const taskCollaborators = reactExports.useMemo(
    () => collaborators.filter((collaborator) => collaborator.task_id === task.id).map(
      (collaborator) => profiles.find((profile) => profile.id === collaborator.collaborator_id)
    ).filter((profile) => Boolean(profile)).filter((profile) => assignableProfiles.some((assignable) => assignable.id === profile.id)),
      ...
      if (dueHasTime) {
        if (dueMoment && dueMoment.getTime() < now.getTime()) {
          const overdueHours = Math.max(
            1,
            Math.ceil((now.getTime() - dueMoment.getTime()) / 36e5)
          );
          return {
            state: "overdue",
  ...
  const addSubtask = async (commentId = null, titleOverride, dueOverride) => {
    ...
    const { data, error } = await supabase.from("subtasks").insert({
      ...
      due_date: null
    }).select("id, task_id, title, done, position, comment_id, due_date, completed_at, assignee_id").single();
    if (error) {
      toast.error(error.message);
      return;
    }
  ...
  const toggleSubtask = async (s) => {
    ...
    setSubtasks(
      (c) => c.map((x) => x.id === s.id ? { ...x, done: nextDone, completed_at: nextCompleted } : x)
    );
  ...
  const moveSubtaskInScope = async (id, dir, commentId) => {
    ...
    await Promise.all(
      reindexed.map(
        (s) => supabase.from("subtasks").update({ position: s.position }).eq("id", s.id)
      )
    );
  };
        ...
        children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "div",
            {
              ...
              style: client?.color ? { background: client.color, color: clientText } : void 0,
              children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-1 border-t px-1.5 py-1", children: [
            ...
            taskPeople.length > 0 ? /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "div",
              {
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "div",
      {
        ...
        children: [
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "min-h-0 flex-1 overflow-visible p-2", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-col gap-0.5", children: [
                      ...
                      children: [
                        /* @__PURE__ */ jsxRuntimeExports.jsx(
                          "span",
                          {
                            ...
                            children: checked ? /* @__PURE__ */ jsxRuntimeExports.jsx(Check, { className: "h-3 w-3" }) : null
                          }
                        ),
                        /* @__PURE__ */ jsxRuntimeExports.jsx(
                          "span",
            ...
            isVisible("description") ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { style: { order: orderOf("description") }, children: descEditing ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mb-2", onPointerDown: stop, onClick: stop, children: /* @__PURE__ */ jsxRuntimeExports.jsx(
              RichTextEditor,
              {
                value: descDraft,
                autoFocus: true,
            ...
            isVisible("attachments") ? /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "div",
              {
                className: "",
                style: { order: orderOf("attachments") },
                children: [
                  attachments.length > 0 ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-3 gap-1", children: [
            ...
            isVisible("priority") ? /* @__PURE__ */ jsxRuntimeExports.jsx(
              "div",
              {
                ...
                children: /* @__PURE__ */ jsxRuntimeExports.jsx(
                  ChipPopover,
                  {
                    ...
                    render: (close) => /* @__PURE__ */ jsxRuntimeExports.jsx(PopoverField, { label: "Prioridade", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
                      Select,
                      {
            ...
            isVisible("meta") ? /* @__PURE__ */ jsxRuntimeExports.jsxs(
              Collapsible,
              {
                ...
                children: [
                  ...
                  /* @__PURE__ */ jsxRuntimeExports.jsx(CollapsibleContent, { className: "border-t p-1.5", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "max-h-56 space-y-0.5 overflow-y-auto", children: assignableProfiles.map((profile) => {
            ...
            isVisible("createdAt") ? /* @__PURE__ */ jsxRuntimeExports.jsx(
              "div",
              {
                className: "flex flex-wrap items-center gap-1",
            ...
            task.completed_at ? /* @__PURE__ */ jsxRuntimeExports.jsx(
              "div",
              {
                className: "flex flex-wrap items-center gap-1",
            ...
            isVisible("meta") ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-0", style: { order: orderOf("meta") }, children: [
            ...
            isVisible("meta") ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-0", style: { order: orderOf("meta") }, children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                CompactRow,
                {
                  ...
                  render: (close) => /* @__PURE__ */ jsxRuntimeExports.jsx(PopoverField, { label: "Responsável", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(
                    Select,
                    {
                      ...
                      children: [
                        ...
                        /* @__PURE__ */ jsxRuntimeExports.jsxs(SelectContent, { children: [
                          ...
                          assignableProfiles.map((p) => /* @__PURE__ */ jsxRuntimeExports.jsx(SelectItem, { value: p.id, children: p.full_name || p.email }, p.id))
                        ] })
                      ]
                    }
                  ) })
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs(DialogFooter, { className: "gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              Button,
              {
                variant: "ghost",
                size: "sm",
                onClick: () => setDueChange({ open: false, pending: null, pendingTime: null, reason: "" }),
                children: "Cancelar"
              }
            ),
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs(DialogFooter, { className: "gap-2", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              Button,
              {
                variant: "ghost",
                size: "sm",
                onClick: () => setSubDueReason({ open: false, subtask: null, prev: null, next: null, reason: "" }),
                children: "Cancelar"
              }
            ),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
...
function TagManagerDialog({ open, onOpenChange }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx(Dialog, { open, onOpenChange, children: /* @__PURE__ */ jsxRuntimeExports.jsxs(DialogContent, { className: "max-w-md", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-3", children: [
      !canManageTags && /* @__PURE__ */ jsxRuntimeExports.jsx("p", { className: "rounded-md border bg-muted/40 p-2 text-xs text-muted-foreground", children: "Apenas administradores podem criar, editar, reordenar ou excluir tags. Estas tags são globais." }),
      canManageTags && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "rounded-md border p-3", children: [
      ...
      canManageTags && /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-[11px] text-muted-foreground", children: [
        "Arraste pelo ícone ",
        /* @__PURE__ */ jsxRuntimeExports.jsx(GripVertical, { className: "inline h-3 w-3 -mt-0.5" }),
        " para reordenar."
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(DndContext, { sensors, collisionDetection: closestCenter, onDragEnd: canManageTags ? onDragEnd : () => {
        ...
        tags.map((tag) => /* @__PURE__ */ jsxRuntimeExports.jsx(TagRow, { tag, onUpdate: update, onDelete: remove, canManage: canManageTags }, tag.id))
      ] }) }) })
    ] })
  ] }) });
}
...
function CompletedColumn({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: isH ? minimal ? "flex w-[clamp(15rem,18vw,19rem)] shrink-0 flex-col" : "flex w-72 shrink-0 flex-col" : "flex w-full flex-col", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("button", { type: "button", onClick: onOpenChange, className: "mb-2 flex w-full items-center gap-1.5 rounded px-1 text-left hover:bg-muted/50", children: [
      ...
      !isH && open && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "ml-2 text-xs text-muted-foreground", children: "Arraste tarefas aqui para concluir" }),
      /* @__PURE__ */ jsxRuntimeExports.jsx(ChevronDown, { className: `ml-auto h-4 w-4 text-muted-foreground transition-transform ${open ? "" : "-rotate-90"}` })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsx(SortableContext, { items: taskIds, strategy: isH ? verticalListSortingStrategy : horizontalListSortingStrategy, children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { ref: setNodeRef, className: `rounded-lg border-2 border-solid p-2 transition ${open ? isH ? "flex flex-col gap-2" : "flex flex-nowrap items-start gap-2 overflow-x-auto pb-2" : "flex items-center justify-center"} ${isOver ? "border-emerald-500 bg-emerald-500/10" : "border-emerald-500/30 bg-emerald-500/5"}`, style: {
      minHeight: open ? isH ? 200 : 120 : 42
    }, children: open ? children : /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-xs font-medium text-muted-foreground", children: "Arraste aqui para concluir" }) }) })
  ] });
}
...
function KanbanPage() {
  ...
  const subtaskDateFilterTaskIds = reactExports.useMemo(() => {
    ...
    return new Set(allSubtasks.filter((subtask) => matchDateFilter({
  ...
  const completedTasks = reactExports.useMemo(() => {
    ...
    return all;
  }, [tasks, filters, sort, sort2, tagNameForTask, completedRange, statuses, user?.id, subtaskAssigneeTaskIds, collaboratorTaskIds, subtaskAssigneeTaskIdsByUser, subtaskDateFilterTaskIds, isCollaborator]);
  ...
  const filtered = reactExports.useMemo(() => {
    ...
    return r;
  }, [tasks, filters, user?.id, subtaskAssigneeTaskIds, collaboratorTaskIds, subtaskAssigneeTaskIdsByUser, subtaskDateFilterTaskIds, isCollaborator]);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex min-h-full flex-col", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("header", { className: "shrink-0 border-b bg-background px-3 py-2", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "mt-2 space-y-1", children: /* @__PURE__ */ jsxRuntimeExports.jsxs(TaskFilters, { filters, onChange: setFilters, hideAssignee: isCollaborator, children: [
...
function KanbanScrollArea({
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex flex-1 flex-col", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { ref: mainRef, className: "kanban-scroll flex-1 overflow-x-auto overflow-y-visible p-4", children }) });
}
export {
