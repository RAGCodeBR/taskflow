...
import { x as cn } from "./router-ZM7179_C.mjs";
...
import { B as Bold, I as Italic, U as Underline, O as Strikethrough, Q as Heading2, R as Heading3, V as List, W as ListOrdered, Y as Quote, Z as Code, _ as Link, g as Eraser, $ as Undo2, a0 as Redo2 } from "../_libs/lucide-react.mjs";
const Popover = Root2;
...
function RichTextView({
  ...
  if (hasEncodedHtmlTag) {
    for (let depth = 0; depth < 3; depth += 1) {
      const decoded = renderedHtml.replace(/&lt;/gi, "<").replace(/&gt;/gi, ">").replace(/&quot;/gi, '"').replace(/&#39;/gi, "'").replace(/&amp;/gi, "&");
  ...
  if (!looksLikeHtml) {
    const formattedHtml = renderedHtml.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/\*\*(.+?)\*\*/g, "<strong>$1</strong>").replace(/(^|[^*])\*([^*]+)\*(?!\*)/g, "$1<em>$2</em>").replace(/\r?\n/g, "<br />");
    return /* @__PURE__ */ jsxRuntimeExports.jsx(
      "div",
      {
        onClick,
