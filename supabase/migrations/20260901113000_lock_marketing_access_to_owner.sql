-- The Marketing audience is defined only by the responsible account.  This is
-- enforced in the RPC and in RLS so another administrator cannot bypass the UI.
...
BEGIN
  IF NOT EXISTS (
    ...
    RAISE EXCEPTION 'Somente o responsável pode definir acessos ao Marketing';
  END IF;
...
CREATE POLICY memberships_admin_manage
  ...
  USING (
    ...
    AND (
      workspace_id <> (SELECT id FROM public.workspaces WHERE slug = 'marketing')
      OR EXISTS (
        SELECT 1 FROM public.profiles
        WHERE id = auth.uid()
          AND lower(email) = 'reinangrupoahouse@gmail.com'
